...
Elm.Main.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var attributes = Elm.Native.Port.make(_elm).inboundSignal("attributes","Json.Decode.Value",function (v) {    return v;});
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "color",_1: model.color}]))]),
      _U.list([$Html.text($Basics.toString(model))]));
   });
   var AttributesChange = function (a) {    return {ctor: "AttributesChange",_0: a};};
   var UpdateCounter = {ctor: "UpdateCounter"};
   var waitAndUpdate = $Effects.task(A2($Task.map,$Basics.always(UpdateCounter),$Task.sleep(1000)));
   var update = F2(function (action,model) {
      var _p0 = action;
      if (_p0.ctor === "UpdateCounter") {
            return {ctor: "_Tuple2",_0: _U.update(model,{count: model.count + 1}),_1: waitAndUpdate};
         } else {
            return {ctor: "_Tuple2",_0: _U.update(model,{color: _p0._0.color}),_1: $Effects.none};
         }
   });
   var Events = function (a) {    return {change: a};};
   var Model = F2(function (a,b) {    return {count: a,color: b};});
   var defaultAttributes = {color: "#0F0"};
   var initialState = {ctor: "_Tuple2",_0: {count: 0,color: defaultAttributes.color},_1: waitAndUpdate};
   var Attributes = function (a) {    return {color: a};};
   var decodeAttributes = A2($Json$Decode.map,Attributes,A2($Json$Decode.at,_U.list(["color"]),$Json$Decode.string));
   var attributesInput = A2($Signal.map,
   AttributesChange,
   A3($Signal.filterMap,$Result.toMaybe,defaultAttributes,A2($Signal.map,$Json$Decode.decodeValue(decodeAttributes),attributes)));
   var app = $StartApp.start({init: initialState,update: update,view: view,inputs: _U.list([attributesInput])});
   var tasks = Elm.Native.Task.make(_elm).performSignal("tasks",app.tasks);
   var main = app.html;
   var events = Elm.Native.Port.make(_elm).outboundSignal("events",
   function (v) {
      return {change: v.change};
   },
   A2($Signal.map,function (model) {    return {change: model.count};},$Signal.dropRepeats(app.model)));
